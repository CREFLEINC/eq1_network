...
class MQTTServerTest:
    ...
    def connect_and_setup(self):
        """브로커 연결 및 초기 설정"""
        try:
            print(f"🔗 MQTT 브로커 연결 중... ({self.mqtt.broker_address}:{self.mqtt.port})")
            self.mqtt.connect()
            print("✅ 연결 성공!")
...
def test_local_broker():
    """로컬 브로커 테스트"""
    print("=== 로컬 MQTT 브로커 테스트 ===")
    test = MQTTServerTest("localhost", 1883)
    test.run_test(20)
...
def interactive_test():
    """대화형 테스트"""
    print("=== 대화형 MQTT 테스트 ===")
    broker = input("브로커 주소 (기본: localhost): ").strip() or "localhost"
