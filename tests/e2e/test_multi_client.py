...
def run_publisher(broker="test.mosquitto.org", port=1883):
    ...
    try:
        mqtt.connect()
        time.sleep(1)  # 연결 대기
        print(f"✅ Publisher 연결 완료: {broker}:{port}")
...
def run_subscriber(broker="test.mosquitto.org", port=1883):
    ...
    try:
        mqtt.connect()
        time.sleep(1)  # 연결 대기
        print(f"✅ Subscriber 연결 완료: {broker}:{port}")
...
def run_monitor(broker="test.mosquitto.org", port=1883):
    ...
    try:
        mqtt.connect()
        time.sleep(1)  # 연결 대기
        print(f"✅ Monitor 연결 완료: {broker}:{port}")
...
if __name__ == "__main__":
    ...
    broker = sys.argv[2] if len(sys.argv) > 2 else "test.mosquitto.org"
