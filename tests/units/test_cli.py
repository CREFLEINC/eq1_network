...
class TestCLI:
    ...
    @patch('app.protocols.mqtt.mqtt_protocol.BrokerConfig')
    @patch('app.protocols.mqtt.mqtt_protocol.ClientConfig')
    def test_test_mqtt_connection_fail(self, mock_client_config, mock_broker_config, mock_mqtt, capsys):
        """MQTT 연결 실패 테스트"""
        mock_mqtt_instance = MagicMock()
    ...
    @patch('app.protocols.mqtt.mqtt_protocol.BrokerConfig')
    @patch('app.protocols.mqtt.mqtt_protocol.ClientConfig')
    def test_test_mqtt_publish_fail(self, mock_client_config, mock_broker_config, mock_mqtt, capsys):
        """MQTT 메시지 발행 실패 테스트"""
        mock_mqtt_instance = MagicMock()
        mock_mqtt_instance.connect.return_value = True
