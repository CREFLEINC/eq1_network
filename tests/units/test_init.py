...
class TestAppInit:
    ...
    def test_successful_imports(self):
        ...
        for item in expected_items:
            ...
            assert in_all or available, f"{item} should be exported via __all__ or available as attribute on app"
    ...
    def test_import_error_handling(self):
        ...
        with patch.dict('sys.modules', {
            ...
            expected_fallback = ['__version__', '__author__', '__email__', '__license__', '__description__']
            assert app.__all__ == expected_fallback

            # API 심볼들이 노출되지 않아야 함(선택적 검증)
            for sym in ['ReqResManager', 'PubSubManager', 'BaseProtocol', 'PubSubProtocol',
                        'ReqResProtocol', 'ProtocolError', 'ProtocolConnectionError',
                        'ProtocolValidationError', 'ProtocolTimeoutError', 'ProtocolDecodeError',
                        'ProtocolAuthenticationError']:
